...
#fill in group info for each student and convert to dummy variable
library(tidyr)
...
increase <-subset(score.improvement, diff > 0)
increase.reg <- lm(data = increase, formula = diff ~ skills + tutoring + refresh + timediff + is.online + is.upper + program.number + 0)
summary(increase.reg)
#conclusion: examine and redesign programs for students who initially have better scores
...
hist(online$diff, main = "Student Improvement Distribution", xlab = "Online")
hist(offline$diff,main = "Student Improvement Distribution", xlab = "Offline")
...
hist(upper$diff,main = "Student Improvement Distribution", xlab = "upper")
hist(lower$diff,main = "Student Improvement Distribution", xlab = "lower")
